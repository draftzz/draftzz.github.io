...
Duas classes de vulnerabilidade empilhadas: race condition + type juggling do PHP. O campo do token fica null entre o INSERT do usuário e a geração do token, e em PHP `[] == null` é true. Os devs patchearam `token=` (403) mas não pensaram no formato array. 50 confirmações por tentativa, 55% de hit (11/20). Janelas minúsculas viram exploráveis de forma confiável quando você joga requisição paralela suficiente em cima.
