...
Two vulnerability classes stacked: race condition + PHP type juggling. The token field is null between user INSERT and token generation, and PHP's `[] == null` is true. Devs patched `token=` (403) but didn't account for the array form. 50 confirms per attempt, 55% hit rate (11/20). Tiny race windows become reliably exploitable once you throw enough parallel requests at them.
